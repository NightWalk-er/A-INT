#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "alint.h"

int main(int argc, char const *argv[]) {
	int n = 12;
	char **a = (char**) malloc(n * sizeof(char*));
	for(int i = 0; i < n; i++) {
		a[i] = (char*) malloc(1001 * sizeof(char));
	}
	
	strcpy(a[0], "1234512345123451234512345");
	strcpy(a[1], "543215432154321543215432154321");
	strcpy(a[2], "0");
	strcpy(a[3], "1234512345123451234512345");
	strcpy(a[4], "1234512345123451234512344");
	strcpy(a[5], "12");
	strcpy(a[6], "265252859812191058636308480000000");
	strcpy(a[7], "265252859812191058636308480000000");
	strcpy(a[8], "5432154321543215432154321");
	strcpy(a[9], "3");
	strcpy(a[10], "");
	strcpy(a[11], "");
	char *result1;
	int index1;

	result1 = alint_add(a[0], a[1]);
	if(!result1) {
		printf("Test alint_add FAILED.\n");
	} else {
		if(0 == strcmp(result1, "543216666666666666666666666666")) {
			printf("Test alint_add PASSED\n");
		} else {
			printf("Test alint_add FAILED.\nYour answer: %s\nExpected answer: %s\n", result1, "543216666666666666666666666666");
		}
		free(result1);
	}

	index1 = alint_compare(a[0], a[1]);
	if(-1 == index1) {
		printf("Test alint_compare PASSED\n");
	} else {
		printf("Test alint_compare FAILED.\nYour answer: %d\nExpected answer: %d\n", index1, -1);
	}

	result1 = alint_diff(a[0], a[1]);
	if(!result1) {
		printf("Test alint_diff FAILED.\n");
	} else {
		if(0 == strcmp(result1, "543214197641976419764197641976")) {
			printf("Test alint_diff PASSED\n");
		} else {
			printf("Test alint_diff FAILED.\nYour answer: %s\nExpected answer: %s\n", result1, "543214197641976419764197641976");
		}
		free(result1);
	}

	result1 = alint_multiply(a[8], a[5]);
	if(!result1) {
		printf("Test alint_multiply FAILED.\n");
	} else {
		if(0 == strcmp(result1, "65185851858518585185851852")) {
			printf("Test alint_multiply PASSED\n");
		} else {
			printf("Test alint_multiply FAILED.\n.Your answer: %s\nExpected answer: %s\n", result1, "65185851858518585185851852");
		}
		free(result1);
	}

	result1 = alint_mod(a[3], a[4]);
	if(!result1) {
		printf("Test alint_mod FAILED.\n");
	} else {
		if(0 == strcmp(result1, "1")) {
			printf("Test alint_mod PASSED\n");
		} else {
			printf("Test alint_mod FAILED.\nYour answer: %s\nExpected answer: %s\n", result1, "1");
		}
		free(result1);
	}

	result1 = alint_mod("978", "5");
	if(!result1) {
		printf("Test alint_mod FAILED.\n");
	} else {
		if(0 == strcmp(result1, "3")) {
			printf("Test alint_mod PASSED\n");
		} else {
			printf("Test alint_mod FAILED.\nYour answer: %s\nExpected answer: %s\n", result1, "3");
		}
		free(result1);
	}

	result1 = alint_mod(a[0], a[5]);
	if(!result1) {
		printf("Test alint_mod FAILED\n");
	} else {
		if(0 == strcmp(result1, "9")) {
			printf("Test alint_mod PASSED\n");
		} else {
			printf("Test alint_mod FAILED.\nYour answer: %s\nExpected answer: %s\n", result1, "9");
		}
		free(result1);
	}

	result1 = alint_pow(a[5], 3);
	if(!result1) {
		printf("Test alint_pow FAILED.\n");
	} else {
		if(0 == strcmp(result1, "1728")) {
			printf("Test alint_pow PASSED\n");
		} else {
			printf("Test alint_pow FAILED.\nYour answer: %s\nExpected answer: %s\n", result1, "1728");
		}
		free(result1);
	}

	result1 = alint_pow("10", 999);
	if(!result1) {
		printf("Test alint_pow FAILED.\n");
	} else {
		if(0 == strcmp(result1, a[10])) {
			printf("Test alint_pow PASSED\n");
		} else {
			printf("Test alint_pow FAILED.\nYour answer: %s\nExpected answer: %s\n", result1, a[10]);
		}
		free(result1);
	}

	result1 = alint_pow("2", 3000);
	if(!result1) {
		printf("Test alint_pow FAILED.\n");
	} else {
		if(0 == strcmp(result1, a[11])) {
			printf("Test alint_pow PASSED\n");
		} else {
			printf("Test alint_pow FAILED.\nYour answer: %s\nExpected answer: %s\n", result1, a[11]);
		}
		free(result1);
	}

	result1 = alint_gcd(a[0], a[5]);
	if(!result1) {
		printf("Test alint_gcd FAILED.\n");
	} else {
		if(0 == strcmp(result1, "3")) {
			printf("Test alint_gcd PASSED\n");
		} else {
			printf("Test alint_gcd FAILED.\nYour answer: %s\nExpected answer: %s\n", result1, "3");
		}
		free(result1);
	}

	result1 = alint_fibonacci(3);
	if(!result1) {
		printf("Test alint_fibonacci FAILED.\n");
	} else {
		if(0 == strcmp(result1, "2")) {
			printf("Test alint_fibonacci PASSED\n");
		} else {
			printf("Test alint_fibonacci FAILED.\nYour answer: %s\nExpected answer: %s\n", result1, "2");
		}
		free(result1);
	}

	result1 = alint_fibonacci(1000);
	if(!result1) {
		printf("Test alint_fibonacci FAILED.\n");
	} else {
		if(0 == strcmp(result1, "43466557686937456435688527675040625802564660517371780402481729089536555417949051890403879840079255169295922593080322634775209689623239873322471161642996440906533187938298969649928516003704476137795166849228875")) {
			printf("Test alint_fibonacci PASSED\n");
		} else {
			printf("Test alint_fibonacci FAILED.\nYour answer: %s\nExpected answer: %s\n", result1, "43466557686937456435688527675040625802564660517371780402481729089536555417949051890403879840079255169295922593080322634775209689623239873322471161642996440906533187938298969649928516003704476137795166849228875");
		}
		free(result1);
	}

	result1 = alint_factorial(30);
	if(!result1) {
		printf("Test alint_factorial FAILED.\n");
	} else {
		if(0 == strcmp(result1, a[6])) {
			printf("Test alint_factorial PASSED\n");
		} else {
			printf("Test alint_factorial FAILED.\nYour answer: %s\nExpected answer: %s\n", result1, a[6]);
		}
		free(result1);
	}

	result1 = alint_factorial(100);
	if(!result1) {
		printf("Test alint_factorial FAILED.\n");
	} else {
		if(0 == strcmp(result1, "93326215443944152681699238856266700490715968264381621468592963895217599993229915608941463976156518286253697920827223758251185210916864000000000000000000000000")) {
			printf("Test alint_factorial PASSED\n");
		} else {
			printf("Test alint_factorial FAILED.\nYour answer: %s\nExpected answer: %s\n", result1, "93326215443944152681699238856266700490715968264381621468592963895217599993229915608941463976156518286253697920827223758251185210916864000000000000000000000000");
		}
		free(result1);
	}

	result1 = alint_bincoeff(10, 8);
	if(!result1) {
		printf("Test alint_bincoeff FAILED.\n");
	} else {
		if(0 == strcmp(result1, "45")) {
			printf("Test alint_bincoeff PASSED\n");
		} else {
			printf("Test alint_bincoeff FAILED.\nYour answer: %s\nExpected answer: %s\n", result1, "45");
		}
		free(result1);
	}

	result1 = alint_bincoeff(1000, 900);
	if(!result1) {
		printf("Test alint_bincoeff FAILED.\n");
	} else {
		if(0 == strcmp(result1, "63850511926305130236698511142022274281262900693853331776286816221524376994750901948920974351797699894319420811933446197797592213357065053890")) {
			printf("Test alint_bincoeff PASSED\n");
		} else {
			printf("Test alint_bincoeff FAILED.\nYour answer: %s\nExpected answer: %s\n", result1, "63850511926305130236698511142022274281262900693853331776286816221524376994750901948920974351797699894319420811933446197797592213357065053890");
		}
		free(result1);
	}

	index1 = alint_max(a, n);
	if(10 == index1) {
		printf("Test alint_max PASSED\n");
	} else {
		printf("Test alint_max FAILED.\nYour answer: %d\nExpected answer: %d\n", index1, 6);
	}

	index1 = alint_min(a, n);
	if(2 == index1) {
		printf("Test alint_min PASSED\n");
	} else {
		printf("Test alint_min FAILED.\nYour answer: %d\nExpected answer: %d\n", index1, 2);
	}

	index1 = alint_search(a, n, a[7]);
	if(6 == index1) {
		printf("Test alint_search PASSED\n");
	} else {
		printf("Test alint_search FAILED.\nYour answer: %d\nExpected answer: %d\n", index1, 6);
	}

	result1 = coin_row_problem(a, n);
	if(!result1) {
		printf("Test coin_row_problem FAILED.\n");
	} else {
		if(0 == strcmp("", result1)) {
			printf("Test coin_row_problem PASSED\n");
		} else {
			printf("Test coin_row_problem FAILED.\nYour answer: %s\nExpected answer: %s\n", result1, "");
		}
		free(result1);
	}
	
	alint_sort(a, n);
	printf("\nTest alint_sort PASSED only if the following sequence of %d alints are sorted in nondecreasing order.\n", n);
	for (int i = 0; i < n; i++) {
		printf("%s\n", a[i]);
	}
	printf("\n");

	index1 = alint_binsearch(a, n, "3");
	if(1 == index1) {
		printf("Test alint_binsearch and probably alint_sort PASSED\n");
	} else {
		printf("Test alint_binsearch and probably alint_sort FAILED.\nYour answer: %d\nExpected answer: %d\n", index1, 1);
	}

	// Don't forget to free all the memory that is dynamically allocated.
	for(int i = 0; i < n; i++) {
		free(a[i]);
	}
	free(a);
	return 0;
}
